...
bool pxq3pe_w_gpio2(struct ptx_card *card, u8 dat, u8 mask)
{
	u8	val;
	struct i2c_msg msg[] = {
		{.addr = 0xB00 | PXQ3PE_I2C_ADR_GPIO,	.flags = I2C_M_RD,	.buf = &val,	.len = 1,},
		{.addr = 0xB00 | PXQ3PE_I2C_ADR_GPIO,	.flags = 0,		.buf = &val,	.len = 1,},
	};
...
void pxq3pe_remove(struct pci_dev *pdev)
{
	...
	/* dma_hw_unmap */
	if (c->dma.dat)
		dma_free_attrs(&pdev->dev, c->dma.sz, c->dma.dat, c->dma.adr, 0);
	if (c->irq_enabled)
...
static struct i2c_algorithm pxq3pe_algo = {
...
static int pxq3pe_probe(struct pci_dev *pdev, const struct pci_device_id *pci_id)
{
	struct ptx_subdev_info	pxq3pe_subdev_info[] = {
		{0x10, TC90522_MODNAME, 0x11, NM131_MODNAME,	{SYS_ISDBT}},
		{0x12, TC90522_MODNAME, 0x13, TDA2014X_MODNAME,	{SYS_ISDBS}},
		{0x14, TC90522_MODNAME, 0x15, NM131_MODNAME,	{SYS_ISDBT}},
		{0x16, TC90522_MODNAME, 0x17, TDA2014X_MODNAME,	{SYS_ISDBS}},
		{0x18, TC90522_MODNAME, 0x19, NM131_MODNAME,	{SYS_ISDBT}},
		{0x1A, TC90522_MODNAME, 0x1B, TDA2014X_MODNAME,	{SYS_ISDBS}},
		{0x1C, TC90522_MODNAME, 0x1D, NM131_MODNAME,	{SYS_ISDBT}},
		{0x1E, TC90522_MODNAME, 0x1F, TDA2014X_MODNAME,	{SYS_ISDBS}},
	};
	...
	c->dma.dat	= dma_alloc_attrs(&pdev->dev, c->dma.sz, &c->dma.adr, GFP_KERNEL, 0);
	if (!c->dma.dat)
	...
	writel(0x0080,	 c->bar + 0x0A00	/*PXQ3PE_DMA_TSMODE*/);				/* port 0 */
	writel(0x0080,	 c->bar + 0x0B40	/*PXQ3PE_DMA_TSMODE + PXQ3PE_DMA_OFFSET_PORT*/);/* port 1 */
	writel(0x0000,	 c->bar + 0x0888);
	writel(0x00CF,	 c->bar + 0x0894);
	writel(0x8000,	 c->bar + 0x088C);
	writel(0x1004,	 c->bar + 0x0890);
	writel(0x0090,	 c->bar + 0x0900);
	writel(0x3200C8, c->bar + 0x0904);
	...
	return err ? ptx_abort(pdev, pxq3pe_remove, err, "Unable to register DVB adapter & frontend (err=%d)", err) : 0;
}
